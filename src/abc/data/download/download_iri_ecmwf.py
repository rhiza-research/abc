...
from abc.utils.data_util import df_contains_multiple_dates, df_contains_nas, is_valid_forecast_date, get_grid
from abc.utils.general_util import (
    download_url,
    dt_to_string,
    get_dates,
    get_folder,
...
# with open(pathlib.Path("/pool001/geflaspo/forecast_rodeo_ii_aux/credentials.json"), "r") as credentials_file:
with open(pathlib.Path("/Users/avocet/content/sheerwater-benchmarking/src/abc/credentials.json"), "r") as credentials_file:
...
for forecast_date in get_dates(args.forecast_dates):

    file_path = folder_name / f"{dt_to_string(forecast_date)}.nc"
    ...
    if file_path.exists() and args.skip_existing:
        print_info(f"Skipping: {day} {month} {
                   year} (file already exists).\n", verbose=args.verbose)
        continue
    ...
    if r.status_code == 200 and r.headers["Content-Type"] == "application/x-netcdf":
        ...
        print_info(f"Data for {day} {month} {
                   year} is not available for model ecmwf.\n", verbose=args.verbose)
    else:
        print_error(bold=True)
        print_info(f"Unknown error occured when trying to download data for {
                   day} {month} {year} for model ecmwf.\n")
